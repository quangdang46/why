...
const DEFAULT_ZAI_BASE_URL: &str = "https://api.z.ai/api/anthropic/v1/messages";
...
pub fn write_config_layer_to_path(config_path: &Path, layer: &WhyConfigLayer) -> Result<()> {
    ...
    let header = r#"# why configuration file
# Generated by `why config init`
# See https://github.com/quangdang46/why for documentation
...
"#;
    ...
    // Add inline comments for each section
    let contents = add_config_comments(&contents);

    fs::write(config_path, format!("{}{}", header, contents))
        .with_context(|| format!("failed to write config file {}", config_path.display()))
}

fn add_config_comments(contents: &str) -> String {
    let mut result = String::new();
    for line in contents.lines() {
        match line {
            "[risk]" => {
                result.push_str("# Risk scoring configuration\n");
                result.push_str("# Controls how why evaluates code change risk\n");
                result.push_str(line);
                result.push('\n');
            }
            "[risk.keywords]" => {
                result.push_str("# Custom keywords that trigger elevated risk levels\n");
                result.push_str(line);
                result.push('\n');
            }
            "[git]" => {
                result.push_str("# Git history analysis settings\n");
                result.push_str(line);
                result.push('\n');
            }
            "[cache]" => {
                result.push_str("# Query result cache settings\n");
                result.push_str(line);
                result.push('\n');
            }
            "[github]" => {
                result.push_str("# GitHub integration settings\n");
                result.push_str(line);
                result.push('\n');
            }
            "[llm]" => {
                result.push_str("# LLM provider configuration\n");
                result.push_str("# Supported providers: anthropic, openai, zai, custom\n");
                result.push_str(line);
                result.push('\n');
            }
            _ => {
