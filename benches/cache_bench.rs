...
fn bench_cache_set(c: &mut Criterion) {
    ...
    for entry_count in [1_usize, 10, 100] {
        group.bench_with_input(BenchmarkId::from_parameter(entry_count), &entry_count, |b, &entry_count| {
            b.iter(|| {
                ...
                for index in 0..entry_count {
                    ...
                    if cache.set(key, format!("report-{index}"), black_box(&head_hash)).is_err() {
                        return;
                    }
                }
            })
...
fn bench_cache_get(c: &mut Criterion) {
    ...
    for entry_count in [1_usize, 10, 100] {
        ...
        let target_key = Cache::make_key("src/auth.rs", &format!("symbol_{}", entry_count / 2), &target_head_hash);

        for index in 0..entry_count {
            let head_hash = format!("{index:016x}");
            let key = Cache::make_key("src/auth.rs", &format!("symbol_{index}"), &head_hash);
            if cache.set(key, format!("report-{index}"), &head_hash).is_err() {
                continue;
            }
        }
